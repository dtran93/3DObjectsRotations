/* 
  CSE 466 Lab 4 Part 3: Rushabh Mehta (1239509), David Tran (1168345)
  This is our code for Part3, roating the cube using the smoothened quaterion data. We used an array of 32 to store the values for averaging.
  We used 32 after much experimentation of various other powers of 2 (to ensure that shifts can be exploited)
  Additionally, we multiplied the roll, yaw & pitch matrices to set the variables that are required to roll the cube.
  It works perfectly on our setup, but may require slight changes in calibration depending on the accelerometer used.
  The front of the cube is red so that it is easier to track the rotations.
  Also, to make sure the screen does not flicker, we overwrite the text in white before printing another line rather than refreshing the entire screen.
 */
...
int colors[] = {ILI9341_RED, ILI9341_RED, ILI9341_RED, ILI9341_RED, ILI9341_BLACK, ILI9341_BLACK, ILI9341_BLACK, ILI9341_BLACK, ILI9341_BLACK, ILI9341_BLACK, ILI9341_BLACK};
#define ScreenColor ILI9341_WHITE
#define halfline 40
...
// Old values stored for erasing the screen cleanly
float yawArray[sizeOfSmoothing];
float rollArray[sizeOfSmoothing];
float pitchArray[sizeOfSmoothing];
int acount = 0;
float yawAvg = 0;
float rollAvg = 0;
float pitchAvg = 0;
...
//    }
  ...
  // For more see http://en.wikipedia.org/wiki/Conversion_between_quaternions_and_Euler_angles which has additional links.
    ...
    // Saving the calculated yaw, pitch and roll in the array for smoothing
     yawArray[(acount) % sizeOfSmoothing] = yaw;
     pitchArray[(acount) % sizeOfSmoothing] = pitch;
     rollArray[(acount) % sizeOfSmoothing] = roll;
     acount++;
     // after some initial, values, we take the average and plot the data
    if (acount > sizeOfSmoothing) {
     oldYawAvg = sumYaw / sizeOfSmoothing;
     oldPitchAvg = sumPitch / sizeOfSmoothing;
     oldRollAvg = sumRoll / sizeOfSmoothing;
     
     sumYaw = sumYaw -  yawArray[(acount) % sizeOfSmoothing] + yaw;
     sumPitch = sumPitch -  pitchArray[(acount)  % sizeOfSmoothing] + pitch;
     sumRoll = sumRoll -  rollArray[(acount) % sizeOfSmoothing] + roll;

      yawAvg = sumYaw / sizeOfSmoothing;
      pitchAvg = sumPitch / sizeOfSmoothing;
      rollAvg = sumRoll / sizeOfSmoothing;
      
      // For printing all the readings on the screen
      // Also, removes all the older values by printing over them in white
        tft.setTextSize(2);
        String temp = "";
        ...
        temp = "Yaw: ";
        temp += oldYawAvg;
        tft.setCursor(0,10); tft.print(temp);
        ...
        temp = "Pitch: ";
        temp += oldPitchAvg;
        tft.setCursor(0,30); tft.print(temp);
        tft.setTextColor(ILI9341_BLACK);
        temp = "Pitch: ";
        temp += pitchAvg;
        tft.setCursor(0,30); tft.print(temp);
        ...
        temp = "Roll: ";
        temp += oldRollAvg;
        tft.setCursor(0,50); tft.print(temp);
        tft.setTextColor(ILI9341_BLACK);
        temp = "Roll: ";
        temp += rollAvg;
        tft.setCursor(0,50); tft.print(temp);
...
void SetVars(void)
{
  ...
  Yan2 = Yan / fact;
  Zan2 = Zan / fact;
  // Zan is modified to account for the roll
  s1 = sin(Yan2);
  s2 = sin(Xan2);
  s3 = sin(Zan2);
  ...
  // all the equations are changed after we multiplied the old ones with the roll matrix
  xx = c3*c1;
  xy = s3*c1;
  xz = -s1;
...
void RenderImage( void)
{
  ...
  for (int i=0; i<LinestoRender; i++ )
  {
    tft.drawLine(Render[i].p0.x,Render[i].p0.y,Render[i].p1.x,Render[i].p1.y, colors[i]);
  }
  OldLinestoRender=LinestoRender;
}
